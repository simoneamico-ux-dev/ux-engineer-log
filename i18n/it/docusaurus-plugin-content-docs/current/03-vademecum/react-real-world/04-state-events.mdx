...
#### Il Sistema degli Eventi Sintetici
...
function CardProdotto({ prodotto, onElimina }) {
  return (
    <div>
      <h3>{prodotto.nome}</h3>
...
#### `e.preventDefault()` (Bloccare il Comportamento Predefinito)
...
`e.preventDefault()` dice al browser di non eseguire il comportamento predefinito, perché sarà il tuo codice a gestire l'azione.
...
#### `e.stopPropagation()` (Fermare la Propagazione)
...
```jsx
function CardArticolo({ articolo, onApriDettagli }) {
  ...
  return (
    <div onClick={onApriDettagli} className="card-cliccabile">
...
#### `e.target` (Elemento che Ha Generato l'Evento)
...
#### Perché le Variabili Locali Non Bastano
...
Se al prossimo render React chiama solo 2 Hook invece di 3 (perché ne hai messo uno dentro un `if`), gli scaffali si scambiano. `eta` finisce per leggere il valore nella posizione sbagliata, e il componente usa uno state che non corrisponde più al nome della variabile.
...
#### Perché `console.log` Può Mostrare il Valore Precedente (Stale Closure)
...
#### Batching (Aggiornamenti Raggruppati)
