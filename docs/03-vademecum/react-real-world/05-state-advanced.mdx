...
React does not compare every internal property of an object. To understand whether state has changed, it first looks at the reference, meaning whether you are passing the same object as before or a new object. If you modify the existing object, the reference stays the same and React may not see the change.

```javascript
// Two different situations for React:

// SITUATION 1: Direct mutation (the reference stays the same)
const user = { name: "Mario", age: 30 };
user.age = 31; // You change the content, but the object is the same
// React checks: "Same reference? Yes → no re-render"

// SITUATION 2: Replacement (the reference changes)
const newUser = { ...user, age: 31 }; // New object, new reference
setUser(newUser); // React checks: "Different reference → RE-RENDER!"
```

The difference is not the value `31`, but the fact that in the second case you created a new object. This reference change is what makes the update visible to React.
...
#### The Updater Function `prev => ...` (Updated State)
...
When you have a form with ten fields, writing ten separate functions becomes repetitive and hard to maintain. There is a way to handle them all with a single function.
...
These methods modify the array "in place", they change the content without creating a new array and without changing the reference. If you update state with the same array, React may not see the change.
...
#### Modifying Elements with `.map()` and Spread
...
### 18. Controlled Components and Forms
...
The pro is very little code and no re-render while the user types. The con is that you lose some practical advantages of controlled state, such as real-time validation and synchronization with other fields.

React 19 introduced a third way with `useActionState`. The idea is to stop fighting the browser and go back to using the native `action` attribute of HTML forms, the same one the web used in 1999 with PHP. React adds state management, pending state, and UI updates.
...
In standard HTML, you select an option by putting `selected` on the individual `<option>`. With many options, this becomes inconvenient. React changes the approach:
...
The practical advantage is that having `category` in state lets you do instant conditional rendering based on the selection.
...
`interests.includes(interest)` is the connection between data and UI. If the value is in the array, the checkmark appears; if it is not, the checkmark disappears. UI and data always stay in sync.
...
`useRef` preserves a value between renders without causing a new render when it changes. It is useful for DOM references, timers, previous values, or technical data the component needs to remember but that should not update the UI.
...
The practical difference is this: `useState` represents data that should update the interface, while `useRef` preserves data that must remain available but should not redraw the component.

-----

#### Under the Hood (`{ current: ... }`)
...
`useEffect` accepts two arguments: the function to execute and a dependency array. The array tells React which values the effect depends on. When one of those values changes between one render and the next, React runs the function again.

Without an array, the effect runs after every render.
...
With an empty array `[]`, React runs the effect after the first render and then does not run it again, unless the component is unmounted and mounted again.
...
With an array containing variables, the effect runs again only when one of those variables changes.
...
#### The True Mental Model (Synchronization)
