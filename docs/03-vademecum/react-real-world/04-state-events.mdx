...
#### `e.preventDefault()` (Blocking the Default Behavior)
...
`e.preventDefault()` tells the browser not to run the default behavior, because your code will handle the action.
...
#### `e.target` (Element That Generated the Event)
...
#### Why Local Variables Are Not Enough
...
If on the next render React calls only 2 Hooks instead of 3 (because you put one inside an `if`), the shelves swap. `age` ends up reading the value in the wrong position, and the component uses state that no longer matches the variable name.
...
#### Why `console.log` Can Show the Previous Value (Stale Closure)
