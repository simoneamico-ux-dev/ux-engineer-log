...
Before writing components, it helps to understand the problem React tries to solve: keeping the UI synchronized with data. This part introduces the mindset shift, JSX, and the way React decides what to update in the browser.
...
In an MPA, the browser requests a new HTML page on every navigation and rebuilds the document. This can produce a temporary white screen between pages: the model works, but the experience can feel less continuous.
...
In a SPA, the document stays the same and JavaScript changes only the view shown to the user. Navigation can therefore feel more continuous, because the browser does not need to download a full new page at every step.
...
#### Curly Braces `{ }` (Static vs. Dynamic)
...
#### Fragment `<>`, a Container Without a DOM Node
...
The **Fragment** solves the problem cleanly: it groups multiple elements to satisfy the single returned value rule, but it does not create an extra DOM node in the browser.
